...
{
    class RBT1<T> where T : IComparable<T>
    {
        //这样免得记混
        ...
        /// <summary>
        /// 添加左旋转，当添加的节点大于上一个节点(即要跑到上一节点右边)时
        /// </summary>
        ...
        private Node Add(Node node, T t)
        {
            ...
            //1.如果出现右子节点是红色，而左子节点是黑色，进行左旋转
            if (IsRed(node.right) && !IsRed(node.left))
                node = LeftRotate(node);

            //如果连续出现左子节点都是红色，进行右旋转
            if (IsRed(node.left) && IsRed(node.left.left))
                node = RightRotate(node);

            //如果出现左右子节点都为红色，进行颜色反转
            if (IsRed(node.right) && IsRed(node.left))
                FlipColor(node);
